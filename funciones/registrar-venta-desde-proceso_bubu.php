...
// $contrato_linea = $datos['linea'];
// $contrato_centro = explode(' ', $datos['documento_tipo'])[1];
...
if(!isset($datos['documento_numero']) or empty($datos['documento_numero']) or $datos['documento_numero'] == 0)
{
  ...
  while($rows_ultimo_numero = $query_ultimo_numero->fetch(PDO::FETCH_ASSOC)) $contrato_numero = $rows_ultimo_numero['contrato_numero']+1;
}
else
{
  $contrato_numero = $datos['documento_numero'];
}
...
foreach ($datos as $value_dato){
    ...
    ,documento_numero,vendedor,solicitud,factura,precio,entrega_inicial,precio_cuota,
     ...
     '".$contrato_numero."','".$value_dato['vendedor']."',
   ...
   // '".$value_dato['documento_numero']."','".$value_dato['vendedor']."',
    ...
    $contrato = $value_dato['linea']."-".$documento_tipo[1]."-".str_pad($contrato_numero,7,"0",STR_PAD_LEFT);
    $total_cuota = 1000;
