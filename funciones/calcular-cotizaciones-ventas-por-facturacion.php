...
if($seleccionado=='true')
  {
    ...
    $consulta = 'SELECT sum(total) as sum_cemen, facturador as facturador FROM facturacion WHERE (fecha >="'.$desde.'" and fecha <="'.$hasta.'") GROUP BY facturador ORDER BY sum_cemen desc';
          ...
          while($rows_resultados = $query_resultados->fetch(PDO::FETCH_ASSOC))
           {
               ...
               if($valor<=$vendedor_desde && $valor>=$vendedor_hasta){
                   ...
                   $json[] = array("year"=>$rows_resultados['facturador'],
                        "facturador" => round(($prod/$tc),0) );
          ...
          while($rows_resultados = $query_resultados->fetch(PDO::FETCH_ASSOC))
           {
               ...
               if($valor<=$vendedor_desde && $valor>=$vendedor_hasta){
                   ...
                   $json[] = array("year"=>$rows_resultados['facturador'],
                        "facturador" => round($rows_resultados['sum_cemen']-0,0));
