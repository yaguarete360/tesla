...
if($seleccionado=='true')
	{
		...
		$consulta = 'SELECT sum(precio) as sum_cemen, producto as producto,vendedor as vendedor FROM ventas WHERE (fecha >="'.$desde.'" and fecha <="'.$hasta.'") GROUP BY vendedor ORDER BY sum_cemen desc';
	        ...
	        while($rows_resultados = $query_resultados->fetch(PDO::FETCH_ASSOC))
           {
               ...
               if($valor<=$vendedor_desde && $valor>=$vendedor_hasta){
                   ...
                   $json[] = array("year"=>$rows_resultados['vendedor'],
                        "vendedor" => round(($prod/$tc),0) );
          ...
          while($rows_resultados = $query_resultados->fetch(PDO::FETCH_ASSOC))
           {
               ...
               if($valor<=$vendedor_desde && $valor>=$vendedor_hasta){
                   ...
                   $json[] = array("year"=>$rows_resultados['vendedor'],
                        "vendedor" => round($rows_resultados['sum_cemen']-0,0));
