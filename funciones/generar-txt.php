<?php if (!isset($_SESSION)) {session_start();}
	...
	// ----------------------------------------------- v FORMA DE USO DE LA FUNCION v -----------------------------------------------
	
	// $ruta_de_archivos = '../../vistas/sintesis/XXXXXXXXXXX/';
	// $nombre_del_archivo = 'XXXXXXXXXXX';
	...
	// $_SESSION['switch_de_estructura_txt'] = 'XXXXXXXXXXX';
	// $contenido_del_txt = "";
	...
	switch ($_SESSION['switch_de_estructura_txt'])
	{
		...
		case 'bancard debito automatico':
			$switch_de_estructura = 'bancard debito automatico';
			include '../../funciones/estructura-txt-bancard.php';
		break;

		case 'cabal debito automatico':
		case 'panal debito automatico':
		case 'procard debito automatico':
			$switch_de_estructura = $_SESSION['switch_de_estructura_txt'];
			include '../../funciones/estructura-txt-'.explode(' ', $_SESSION['switch_de_estructura_txt'])[0].'.php';
		break;

		case 'itau debito automatico':
			$switch_de_estructura = $_SESSION['switch_de_estructura_txt'];
			include '../../funciones/estructura-txt-itau-debitos-automaticos.php';
		break;
				   ...
				   //              $valor_largo_controlado = substr($salario_imponible + 0, $comienzo_del_substr, $estructura['largo']);
					...
					case 'bancard debito automatico':
						switch ($campo_nombre)
						{
							...
							case 'FecTran':
							case 'NroLote':
								$valor_largo_controlado = substr($monto[$campo_nombre], $comienzo_del_substr, $estructura['largo']);
								$linea_texto.= str_pad($valor_largo_controlado, $estructura['largo'], $caracter_pad, $direccion_pad);
							break;

							case 'importe':
								$valor_largo_controlado = substr(number_format($monto[$campo_nombre], $estructura['decimales'], '', ''), $comienzo_del_substr, $estructura['largo']);
								$linea_texto.= str_pad($valor_largo_controlado, $estructura['largo'], $caracter_pad, $direccion_pad);
							break;
						}
					break;
