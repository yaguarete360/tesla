...
foreach ($datos as $value_dato){
    ...
    $contrato_numero = 0;
    $contrato_linea = $value_dato['linea'];
    $contrato_centro = explode(' ', $value_dato['documento_tipo'])[1];
    $consulta_ultimo_numero = 'SELECT contrato_numero FROM contratos WHERE borrado = "no" AND contrato_linea = "'.$contrato_linea.'" AND contrato_centro = "'.$contrato_centro.'" ORDER BY contrato_numero DESC LIMIT 1';
    /*$file = fopen('ULTIMO_NUMERO.txt', "w");
    fwrite($file, $consulta_ultimo_numero . PHP_EOL);
    fclose($file);*/
    $query_ultimo_numero = $conexion->prepare($consulta_ultimo_numero);
    $query_ultimo_numero->execute();
    $query_ultimo_numero_resultado = $query_ultimo_numero->fetchAll();

    foreach ($query_ultimo_numero_resultado as $valor_ultimo_documento){
        $contrato_numero = $valor_ultimo_documento['contrato_numero']+1;
    }
    $file = fopen('ULTIMO_NUMERO.txt', "w");
    fwrite($file, $consulta_ultimo_numero . PHP_EOL);
    fclose($file);
    ...
    if($value_dato['cuotas']=="contado"){
        $_SESSION['cuotas'] = 0;
    }else{
        $_SESSION['cuotas'] = $value_dato['cuotas'];
    }


    $_SESSION['pagare_numero'] = "sin datos";
    if(!empty($value_dato['pagare_numero'])){
        $_SESSION['pagare_numero'] = $value_dato['pagare_numero'];
    }

    $_SESSION['factura'] = "sin datos";
    if(!empty($value_dato['factura'])){
        $_SESSION['factura'] = $value_dato['factura'];
    }

    $documento_varchar = str_pad($contrato_numero,7,"0",STR_PAD_LEFT);
    $_SESSION['documento_numero_varchar'] = $documento_varchar;
    $_SESSION['documento_numero'] = $contrato_numero;
    ...
    ,documento_numero,documento_numero_varchar,vendedor,solicitud,factura,precio,entrega_inicial,precio_cuota,
     ...
     '".$_SESSION['documento_numero']."','".$_SESSION['documento_numero_varchar']."','".$value_dato['vendedor']."',
     '".$value_dato['solicitud']."', '".$_SESSION['factura']."', '".str_replace(",","",$value_dato['precio'])."',
    ...
    $contrato = $value_dato['linea']."-".$documento_tipo[1]."-".str_pad($_SESSION['documento_numero'],7,"0",STR_PAD_LEFT);
    $total_cuota = 1000;
    ...
    if($existe==0){
        ...
        $sql_insert = "INSERT INTO contratos (
                   ...
                   'fecha nacimeinto del portador',
                   'sexo del portador',
                   'estado civil del portador',
                   '0000-00-00',
                   'edad del portador',
                   'sin datos',
                   'sin datos',
...
$mensaje_2 = array("mensaje"=>$mensaje,"contrato"=>$contrato);
