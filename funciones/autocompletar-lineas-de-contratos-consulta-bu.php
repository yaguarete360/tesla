...
$consulta_autocompletar = 'SELECT id, contrato, descripcion, cuota, obligacion, derecho, cuota_vencimiento, efectuado_fecha FROM diario
    WHERE borrado = "no"
...
while($rows_autocompletar = $query_autocompletar->fetch(PDO::FETCH_ASSOC))
{
    ...
    else
    {
    	...
    	else
    	{
    		...
    		$consulta_pagos_de_la_cuota = 'SELECT SUM(obligacion) AS suma_obligacion, efectuado_fecha FROM diario
			    WHERE borrado = "no"
			    	AND efectuado_fecha != "0000-00-00"
				    AND documento_tipo LIKE "contrato%"
				    AND descripcion NOT LIKE "comision%"
